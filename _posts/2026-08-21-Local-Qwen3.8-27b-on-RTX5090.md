---
layout:     post
title:      "Running Qwen3.8-27b with maximum context on a single RTX5090"
...
![deepseekadjustments](/images/2026/Qwen3.8-5090/deepseek-increases.png)



Still cheap, but naw fuck that.
...
The open weights for [Qwen 3.8][4] was just released last week and I was extremely keep to get my hands on it. Benchmarks never tell the full story, but it's a good starting point if you want to gauge a model. A 27b model that fits on consumer hardware, trading blows with Opus 4.6, really? Interesting
...
Models are not particularly small. So models get quantized. A Q4 model is about half the size of a Q8. A Q6 would be somewhere in between. But this is just a broad generalization. I won't go into `Q` vs `FP` vs `IQ` or `NVFP4`, let's just focus on the number suffix. 
...
The original Qwen3.8-27b model released is 56gb in size. It's not going to fit in our little gaming graphics cards.



Unsloth is one of the more reputable sources of quantized models in this. On [their huggingface][9] you can see a ton of quants available.
...
Can I run Qwen on a 8GB VRAM GPU? Sure, in theory you can squeeze extremely low-bit variants into a 8GB GPU, but you would have left almost no room for runtime overhead and KV cache. It could run, but you would be severely limited in how much you can do with it.
...
I gave up. I'm not good enough to get a working version going with the maximum context possible with vLLM or SGLang. Plus, I'm on Windows on my personal gaming PC. I have additional overhead on top of all that.
...
Do I really need  262144 context? Potentially not, but self satisfaction knowing I did it is good for mental health.
...
Model: `unsloth/Qwen3.8-27B-GGUF` - `UD_Q4_K_M` variant. (the 18gb one)
...
KV Cache DType: `Q4_0` - This is important as it reduces KV VRAM allocation significantly
...
And in `opencode.json`, add the following:
...
~~~
    "unsloth-studio": {
      ...
      "name": "Unsloth-Local",
      "options": {
        "baseURL": "http://192.168.0.7:8888/v1/",
        "apiKey": "sk-unsloth-df57d671ebce4771c8af4eaec422c433"
      },
      "models": {
